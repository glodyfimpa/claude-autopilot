...
| `/autopilot-configure` | First-run wizard for the full pipeline: picks providers for PRD source, task storage, PR target, parallelization, code quality |
...
Quick stats: `bats tests/lib/ | tail -1` for test count, `ls lib/*-providers/*.sh | wc -l` for provider count.
...
Current state: 373 tests, all green on macOS bash 3.2.
...
`scripts/release.sh` is the canonical release flow. It bumps `.claude-plugin/plugin.json`, syncs the `Current state: NNN tests` line in this README, builds an annotated tag with a draft changelog from PR-merge commits, pushes, and creates the GitHub release in one shot.

```
scripts/release.sh 0.7.0              # full flow with $EDITOR pass on annotation
scripts/release.sh 0.7.0 --no-edit    # skip editor, use autogenerated changelog
scripts/release.sh 0.7.0 --dry-run    # print every planned action; mutate nothing
```

Preconditions enforced before any mutation: clean working tree, on `main`, target version greater than current. The bats suite runs first; a single failure aborts the release.
